...
| `load_1m`       | `double`              | The current system load average over the last one minute. This is useful for making [load-based checks](#using-the-sytem-load-average).      |
| `load_5m`       | `double`              | The current system load average over the last five minutes. This is useful for making [load-based checks](#using-the-sytem-load-average).    |
| `load_15m`      | `double`              | The current system load average over the last fifteen minutes. This is useful for making [load-based checks](#using-the-sytem-load-average). |
