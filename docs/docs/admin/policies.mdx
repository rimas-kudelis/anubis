...
As Anubis works through determining if a user is legitimate or not, it needs to store temporary data. Anubis can do this via one of a few storage backends. Each backend has its own advantages and disadvantages.
...
If no storage backend is set in [the policy file](../policies.mdx), Anubis will use the [`memory`](#memory) backend by default. This is equivalent to the following in the policy file:
